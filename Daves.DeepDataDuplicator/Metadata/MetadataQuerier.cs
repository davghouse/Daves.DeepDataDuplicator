...
@"SELECT
    ...
    CONVERT(bit, case t.name when 'timestamp' then 1 else 0 end) isRowVersion
FROM sys.columns c
JOIN sys.types t
...
WHERE parent_object_id IN (SELECT object_id FROM sys.tables)";
        ...
        public virtual IReadOnlyList<Column> QueryColumns()
            => Query(ColumnQuery,
                r => new Column(r["tableId"], r["name"], r["columnId"], r["isNullable"], r["isIdentity"], r["isComputed"], r["isRowVersion"]))
            .ToReadOnlyList();
